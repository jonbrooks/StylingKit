...
- (NSArray *)viewStylers
{
    ...
    dispatch_once(&onceToken, ^{
        stylers = @[
                    ...
                    [[PXGenericStyler alloc] initWithHandlers: @{@"text-decoration" : ^(PXDeclaration *declaration, PXStylerContext *context) {
                        STStyleable *view = (STStyleable *)context.styleable;
                        view.textDecoration = declaration.stringValue;
                    }}],
                    [[PXGenericStyler alloc] initWithHandlers: @{@"text-align" : ^(PXDeclaration *declaration, PXStylerContext *context) {
                        STStyleable *view = (STStyleable *)context.styleable;
                        view.textAlign = declaration.stringValue;
                    }}],
                    [[PXGenericStyler alloc] initWithHandlers: @{@"gradient-color" : ^(PXDeclaration *declaration, PXStylerContext *context) {
                        STStyleable *view = (STStyleable *)context.styleable;
                        view.gradientColors = declaration.stringValue;
                    }}],
                    [[PXGenericStyler alloc] initWithHandlers: @{@"gradient-color-position" : ^(PXDeclaration *declaration, PXStylerContext *context) {
                        STStyleable *view = (STStyleable *)context.styleable;
                        view.gradientColorsPositions = declaration.stringValue;
                    }}]
        ];
    });
...
- (void)updateStyleWithRuleSet:(PXRuleSet*)ruleSet context:(PXStylerContext*)context {
    ...
    id<PXPaint> paint = context.fill;
    
    if ([paint isKindOfClass:[PXSolidPaint class]])
    {
        self.backgroundColor = ((PXSolidPaint *)paint).color;
    }
